...
(async()=>{
 ...
 const captureFinish=JSON.stringify(b.states.at(-1).capture);send(a,{type:'mode',mode:'deathmatch'});await until(()=>b.states.at(-1).nextMode==='deathmatch','next mode selection');assert.equal(b.states.at(-1).mode,'capture');assert.equal(JSON.stringify(b.states.at(-1).capture),captureFinish);send(a,{type:'mode',mode:'tdm'});await until(()=>b.states.at(-1).nextMode==='tdm','choose next team deathmatch');
