...
(async()=>{
 ...
 send(b,{type:'mode',mode:'deathmatch'});await delay(100);assert.equal(b.states.at(-1).mode,'capture');send(a,{type:'mode',mode:'deathmatch'});await until(()=>b.states.at(-1).mode==='deathmatch','host changes mode');send(a,{type:'mode',mode:'capture'});await until(()=>b.states.at(-1).mode==='capture','host restores capture');assert.notEqual(b.states.at(-1).players[0].team,b.states.at(-1).players[1].team);
 ...
 send(a,{type:'switch',slot:1});await until(()=>a.states.at(-1)?.self.slot===1,'weapon switch');assert.equal(b.states.at(-1).capture.points.length,3);send(a,{type:'mode',mode:'deathmatch'});await delay(80);assert.equal(b.states.at(-1).mode,'capture');console.log('PASS live round and server weapon switching');
 ...
 send(a,{type:'input',seq:10,crouch:true});await until(()=>a.states.at(-1).players.find(p=>p.id===a.id).stance==='crouch','crouch replication');send(a,{type:'input',seq:11});assert.ok(Math.abs(a.states.at(-1).players[0].characterScale-1/1.5)<1e-9);console.log('PASS stance and Bellhaven character size replicate over WebSocket');
 ...
 const captureFinish=JSON.stringify(b.states.at(-1).capture);send(a,{type:'mode',mode:'deathmatch'});await until(()=>b.states.at(-1).nextMode==='deathmatch','next mode selection');assert.equal(b.states.at(-1).mode,'capture');assert.equal(JSON.stringify(b.states.at(-1).capture),captureFinish);send(a,{type:'mode',mode:'capture'});await until(()=>b.states.at(-1).nextMode==='capture','restore next capture mode');
